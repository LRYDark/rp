...
function rpShowMonth(formId, toupdate, monthNames, year, month) {
    $.each(monthNames, function (index, val) {
        var monthClass = 'btn btn-info rp-button';
        if (index == 1) {
            monthClass = 'btn btn-info rp-button rp-corner-left';
        }
        
        if (index == 12) {
            monthClass = 'btn btn-info rp-button rp-corner-right';
        }
        if (month == index) {
           monthClass = 'btn btn-info rp-button rp-state-active';
        } 
        $('<a class="' + monthClass + '" href="#' + index + '_' + year + '">' + val + '</a>').click(function () {
            if (!$(this).hasClass('rp-state-disabled')) {
                $(this).removeClass('rp-state-hover');
            }
            searchRp(index, formId, year, monthNames);

        }).mousedown(function () {
            $(this).not('.rp-state-active')
                    .not('.rp-state-disabled')
                    .addClass('rp-state-down');
        }).mouseup(function () {
            $(this).removeClass('rp-state-down');
        }).hover(
                function () {
                    $(this).not('.rp-state-active')
                            .not('.rp-state-disabled')
                            .addClass('rp-state-hover');
                },
                function () {
                    $(this).removeClass('rp-state-hover')
                            .removeClass('rp-state-down');
                }
        ).appendTo(toupdate);
        
    });
}
function searchRp(index, formId, year, monthNames) {
...
function lastYearManagesEntities(formId, element, year, monthNames) {
    ...
    $('<a class=\"' + monthClass + ' rp_href\"  href=\"#' + (year - 1) + '\">' + (year - 1) + '</a>').click(function () {
        if (!$(this).hasClass('fc-state-disabled')) {
            $(this).removeClass('fc-state-hover');
        }
        searchRp(12, formId, year-1, monthNames);
    }).mousedown(function () {
        $(this).not('.rp-state-active')
                .not('.rp-state-disabled')
                .addClass('rp-state-down');
...
function nextYearManagesEntities(formId, element, year, monthNames) {
    ...
    $('<a class=\"' + monthClass + ' rp_href\"  href=\"#' + (year + 1) + '\">' + (year + 1) + '</a>').click(function () {
        ...
        searchRp(1, formId, year+1, monthNames);
    }).mousedown(function () {
        $(this).not('.rp-state-active')
                .not('.rp-state-disabled')
                .addClass('rp-state-down');
