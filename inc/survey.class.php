...
/**
 * Class PluginRpSurvey
 */
class PluginRpSurvey extends CommonDBTM {
   ...
   static function getTypeName($nb = 0) {
      return _n('Rp survey', 'Rp surveys', $nb, 'rp');
   }
   ...
   function prepareInputForAdd($input) {
      ...
      if ($input['is_active'] == 1) {
         ...
         if (count($found) > 0) {
            Session::addMessageAfterRedirect(__('Error : only one survey is allowed by entity', 'rp'), false, ERROR);
            return false;
         }
      }
   ...
   function pre_deleteItem() {
      ...
      $question = new PluginRpSurveyQuestion;
      $question->deleteByCriteria([PluginRpSurveyQuestion::$items_id => $this->getID()]);

      $answer = new PluginRpSurveyAnswer;
      $answer->deleteByCriteria([PluginRpSurveyAnswer::$items_id => $this->getID()]);

      $reminder = new PluginRpSurveyReminder();
      $reminder->deleteByCriteria([PluginRpSurveyReminder::$items_id => $this->getID()]);
   ...
   function duplicateSurvey($ID, $entities_id) {
      ...
      $question_obj  = new PluginRpSurveyQuestion();
      $questions = $question_obj->find(['plugin_rp_surveys_id' => $ID]);
      $questions = toolbox::addslashes_deep($questions);
      foreach ($questions as $question) {
         $question['plugin_rp_surveys_id'] = $newID;
         ...
         $translation_obj  = new PluginRpSurveyTranslation();
         $translations = $translation_obj->find([
            'plugin_rp_surveys_id' => $ID,
            'glpi_plugin_rp_surveyquestions_id' => $question_id
         ]);
