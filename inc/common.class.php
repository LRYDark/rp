...
abstract class PluginRpCommon extends CommonGLPI {
   ...
   **/
   final function addStandardTab($itemtype, &$ong, $options) {
      ...
      if (!is_integer($itemtype)
         ...
         if (method_exists($itemtype, "displayTabContentForRP")
             && !($obj instanceof PluginRpCommon)) {
   ...
   **/
   function defineAllTabsRP($options=[]) {
   ...
   **/
    ...
    * is run first, before displayCommonTabForRP.
    *
    * @since version 0.83
    *
    * @param $rp          PluginRpSimpleRP object for output
    * @param $item         CommonGLPI object for which the tab need to be displayed
    * @param $tab   string tab number
    *
    * @return true if display done (else will search for another handler)
   **/
   static function displayTabContentForRP(PluginRpSimpleRP $rp, CommonGLPI $item, $tab) {
      return false;
   }
    ...
    * @return true if display done (else will search for another handler)
   **/
   static final function displayCommonTabForRP(PluginRpSimpleRP $rp, CommonGLPI $item, $tab) {

      switch ($tab) {
         case $item->getType().'$main' :
            static::rpMain($rp, $item);
            break;
         ...
         case 'Document_Item$1' :
            if (Session::haveRight('document', READ)) {
               PluginRpDocument::rpForItem($rp, $item);
            }
            break;

         case 'NetworkPort$1' :
            PluginRpNetworkPort::rpForItem($rp, $item);
            break;

         case 'Infocom$1' :
            if (Session::haveRight('infocom', READ)) {
               PluginRpInfocom::rpForItem($rp, $item);
            }
            break;

         case 'Contract_Item$1' :
            if (Session::haveRight("contract", READ)) {
               PluginRpContract_Item::rpForItem($rp, $item);
            }
            break;

         case 'Ticket$1' :
            if (Ticket::canView()) {
               PluginRpItem_Ticket::rpForItem($rp, $item);
            }
            break;

         case 'Item_Problem$1' :
            if (Problem::canView()) {
               PluginRpItem_Problem::rpForItem($rp, $item);
            }
            break;

         case 'Change_Item$1' :
            if (Change::canView()) {
               PluginRpChange_Item::rpForItem($rp, $item);
            }
            break;

         case 'ManualLink$1' :
            if (Session::haveRight('link', READ)) {
               PluginRpLink::rpForItem($rp, $item);
            }
            break;

         case 'Reservation$1' :
            if (Session::haveRight('reservation', READ)) {
               PluginRpReservation::rpForItem($rp, $item);
            }
            break;

         case 'Log$1' :
            PluginRpLog::rpForItem($rp, $item);
            break;

         case 'KnowbaseItem_Item$1' :
            if (KnowbaseItem::canView()) {
               PluginRpItem_Knowbaseitem::rpForItem($rp, $item);
            }
            break;

         case 'Item_Devices$1' :
            if (Session::haveRight('device', READ)) {
               PluginRpItem_Device::rpForItem($rp, $item);
            }
            break;

         case 'Item_Disk$1' :
            PluginRpItem_Disk::rpForItem($rp, $item);
            break;

         case 'Computer_Item$1' :
            PluginRpComputer_Item::rpForItem($rp, $item);
            break;

         case 'Item_SoftwareVersion$1' :
            PluginRpItem_SoftwareVersion::rpForItem($rp, $item);
            break;

         Case 'Domain_Item$1' :
            PluginRpDomain_Item::rpForItem($rp, $item);
            break;

         case 'Item_OperatingSystem$1' :
            PluginRpItem_OperatingSystem::rpForItem($rp, $item);
            break;
   ...
   **/
   static function displayTabContentForItem(CommonGLPI $item, $tabnum=1, $withtemplate=0) {
      ...
      $pref = new PluginRpPreference;
      $pref->menu($item, Plugin::getWebDir('rp')."/front/export.php");
   ...
   **/
   ...
   static function rpNote(PluginRpSimpleRP $rp, CommonDBTM $item) {
      ...
      $rp->setColumnsSize(100);
      $title = '<b>'._n('Note', 'Notes', $number).'</b>';

      if (!$number) {
         $rp->displayTitle(sprintf(__('%1$s: %2$s'), $title, __('No item to display')));
      } else {
   ...
   **/
   final function generateRP($tab_id, $tabs, $page=0, $render=true) {

      $dbu = new DbUtils();

      $this->rp = new PluginRpSimpleRP('a4', ($page ? 'landscape' : 'portrait'));
      ...
      foreach ($tab_id as $key => $id) {
         ...
         foreach ($tabs as $tab) {
            if (!$this->displayTabContentForRP($this->rp, $this->obj, $tab)
                && !$this->displayCommonTabForRP($this->rp, $this->obj, $tab)) {
               ...
               if (!is_integer($itemtype)
                  ...
                  if ($itemtype == "Item_Devices") {
                     $PluginRpComputer = new PluginRpComputer();
                     if ($PluginRpComputer->displayTabContentForRp($this->rp, $this->obj,
                                                                     $tabnum)) {
                        continue;
                     }
                  } else if (method_exists($itemtype, "displayTabContentForRp")
                             && ($obj = $dbu->getItemForItemtype($itemtype))) {
                     if ($obj->displayTabContentForRp($this->rp, $this->obj, $tabnum)) {
                        continue;
                     }
                  }
               }
               Toolbox::logInFile('php-errors',
                                  sprintf(__("RP: don't know how to display %s tab").'\n', $tab));
            }
         }
      }
      $config = PluginRpConfig::getInstance();
      if (!empty($config->getField('add_text'))) {
         $this->rp->displayText('<b><i>'.$config->getField('add_text').'</i></b>', '', 5);
      }

      if($render) {
         $this->rp->render();
      } else {
         return $this->rp->output();
      }
   }


   static function mainTitle(PluginRpSimpleRP $rp, $item) {

      $rp->setColumnsSize(50,50);
      ...
      return $rp->displayTitle($col1, $col2);
   }


   static function mainLine(PluginRpSimpleRP $rp, $item, $field) {
      ...
      switch($field) {
         ...
         case 'comment' :
            return $rp->displayText('<b><i>'.sprintf(__('%1$s: %2$s'), __('Comments').'</i></b>',
                                                      ''), $item->fields['comment']);
