...
function rp_scan_bl_result(array $row, string $gestion_webdir, string $rootdoc): array {
   ...
   if ($signed === 1) {
       ...
       */
      // Le combiné n'est annoncé que si l'utilisateur peut produire le rapport
      // (porte atelier comprise) ; gestion/ajax/cri.php retombe de toute
      // façon sur le bon seul sinon.
      $combined    = $tickets_id > 0 && PluginRpAccess::canProduce('rapport_tech');
      $sign_params = [
         'job'        => $tickets_id,
